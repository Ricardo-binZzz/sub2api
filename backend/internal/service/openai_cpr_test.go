...
func TestCPRSharesOAuthUpstreamSemantics(t *testing.T) {
	...
	now := time.Now()
	cprRate, cprOK := openAISchedulingRate(cpr, now, float64Ptr(0.35))
	oauthRate, oauthOK := openAISchedulingRate(oauth, now, float64Ptr(0.35))
	require.True(t, cprOK)
	require.Equal(t, oauthOK, cprOK)
	require.Equal(t, oauthRate, cprRate)
	apikeyRate, apikeyOK := openAISchedulingRate(apikey, now, float64Ptr(0.35))
	require.True(t, apikeyOK)
	require.Equal(t, 1.0, apikeyRate, "apikey without a billing probe keeps its default account multiplier")
...
func TestCPRJoinsOpenAIUpstreamCostPool(t *testing.T) {
	...
	factors := openAIUpstreamCostFactors([]*Account{apikey, cpr}, now, float64Ptr(0.35))
	...
	order := newOpenAILegacyUpstreamRateOrder([]*Account{apikey, cpr}, now, float64Ptr(0.35))
	...
	only := openAIUpstreamCostFactors([]*Account{apikey}, now, float64Ptr(0.35))
	require.Equal(t, openAIUpstreamCostNeutralFactor, only[apikey.ID])
	require.False(t, newOpenAILegacyUpstreamRateOrder([]*Account{apikey}, now, float64Ptr(0.35)).enabled)
}
