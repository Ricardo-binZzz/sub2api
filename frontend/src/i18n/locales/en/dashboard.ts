export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      sdk: {
        description: 'Generate ready-to-run examples for the OpenAI Python SDK, Node.js SDK, and curl with this key.',
        secretHint: 'This example contains the current API key. Do not commit it or share it with others.',
      },
      antigravity: {
  ...
  usage: {
    ...
    cacheAnalysis: {
      title: 'Prompt Cache Analysis',
      subtitle: 'Calculated from the active time range and filters',
      tokenHitRate: 'Token hit rate',
      reuseRatio: 'Cache reuse ratio',
      cacheReadVolume: 'Cache read volume',
      cacheWriteVolume: 'Cache creation volume',
      promptTokens: 'Prompt tokens',
      diagnosis: 'Diagnosis',
      noData: 'No prompt tokens are available for analysis in this range',
      status: {
        no_data: 'No data',
        inactive: 'Cache inactive',
        low: 'Low hit rate',
        churn: 'Frequent cache rebuilds',
        fair: 'Room to improve',
        good: 'Healthy cache usage',
      },
      description: {
        no_data: 'The selected range contains no tokens that can be analyzed for cache usage.',
        inactive: 'Requests produced no cache reads or writes. Check whether the upstream model supports caching and whether the cache policy is enabled.',
        low: 'Cache reads are present, but they cover only a small share of prompt tokens. Check stable prefixes, cache keys, and account affinity.',
        churn: 'Cache creation exceeds cache reads, so cached prefixes may not be reused enough. Reduce prefix changes and avoid frequent account switches within a session.',
        fair: 'The cache is being reused, with additional gains available from stable prompt prefixes and session routing.',
        good: 'Most prompt tokens are served by cache reads, so the current strategy is working well.',
      },
    },
