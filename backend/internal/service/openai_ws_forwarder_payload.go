...
func (s *OpenAIGatewayService) buildOpenAIWSHeaders(
	...
	if account != nil && account.UsesOpenAICodexProtocol() {
		// Header overrides and the ingress allow-list are not a trust boundary:
		// remove all client/private identity channels after the last override, then
		// restore only the protocol values generated by this gateway.  Keep the
		// already-isolated session/conversation pair for device/off modes; session
		// and full fingerprint modes replace the session with the account snapshot.
		sessionID := strings.TrimSpace(headers.Get("session_id"))
		conversationID := strings.TrimSpace(headers.Get("conversation_id"))
		turnStateValue := strings.TrimSpace(headers.Get(openAIWSTurnStateHeader))
		ids := stagedCodexFingerprintIDs(c, account)
		sanitizeCodexOutboundIdentityHeaders(headers, ids)
		...
		if turnStateValue != "" {
			headers.Set(openAIWSTurnStateHeader, turnStateValue)
		...
		headers.Set("OpenAI-Beta", betaValue)
		// The session feature header is gateway-owned.  Rebuild it after stripping
		// any client declaration so an opaque feature list cannot become a
		// fingerprint/routing channel.
		applyOpenAICodexBetaFeatures(c, account, headers)
