...
func (s *OpenAIQuotaService) referralCall(ctx context.Context, id int64, program string) (OpenAIReferralCall, error) {
	...
	upstreamCall, err := s.prepareUpstreamCall(ctx, id, false)
	if err != nil {
		return OpenAIReferralCall{}, err
	}
	headers, _, err := s.buildCodexQuotaHeaders(upstreamCall)
	if err != nil {
	...
	return OpenAIReferralCall{ProxyURL: upstreamCall.proxyURL, Headers: headers, ProgramID: program}, nil
}
