<template>
  <div class="min-h-screen bg-gray-50 text-gray-900 dark:bg-dark-950 dark:text-white">
    <header class="sticky top-0 z-40 border-b border-gray-200/80 bg-white/95 backdrop-blur dark:border-dark-700 dark:bg-dark-900/95">
      <div class="mx-auto flex h-16 max-w-[1500px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <router-link to="/home" class="flex min-w-0 items-center gap-3">
          <img :src="siteLogo" alt="" class="h-9 w-9 rounded-lg object-contain shadow-sm" />
          <div class="hidden truncate text-base font-semibold min-[360px]:block sm:text-lg">{{ siteName }}</div>
        </router-link>
        ...
        <div class="flex items-center gap-1.5 sm:gap-2">
          ...
          <button
            type="button"
            class="flex h-9 w-9 items-center justify-center rounded-lg text-gray-500 hover:bg-gray-100 dark:text-dark-300 dark:hover:bg-dark-800"
            :title="isDark ? '切换到浅色模式' : '切换到深色模式'"
            @click="toggleTheme"
          >
            <Icon :name="isDark ? 'sun' : 'moon'" size="md" />
          </button>
          <router-link :to="primaryPath" class="btn btn-primary ml-1">
    ...
    <div class="mx-auto grid max-w-[1500px] grid-cols-1 lg:grid-cols-[260px_minmax(0,1fr)]">
      <aside class="hidden border-r border-gray-200 px-6 py-12 dark:border-dark-700 lg:block">
        <div class="sticky top-28">
          <div class="mb-4 px-3 text-xs font-semibold text-gray-400 dark:text-dark-500">快速上手</div>
          <nav class="space-y-1 text-sm" aria-label="教程目录">
            <a
              v-for="(item, index) in toc"
              :key="item.href"
              :href="item.href"
              class="group relative grid grid-cols-[28px_1fr] items-center gap-2 rounded-lg px-3 py-3 text-gray-600 transition-colors hover:bg-white hover:text-gray-900 dark:text-dark-300 dark:hover:bg-dark-900 dark:hover:text-white"
              :class="index === 0 ? 'border border-gray-200 bg-white text-gray-900 shadow-sm dark:border-dark-700 dark:bg-dark-900 dark:text-white' : ''"
            >
              <span v-if="index === 0" class="absolute -left-9 h-7 w-1 rounded-r bg-primary-500"></span>
              <span class="font-mono text-[11px] text-gray-400 group-hover:text-primary-600 dark:text-dark-500">{{ String(index + 1).padStart(2, '0') }}</span>
              <span>{{ item.label }}</span>
            </a>
          </nav>

          <div class="mt-10 rounded-lg border border-gray-200 bg-white p-4 dark:border-dark-700 dark:bg-dark-900">
            <Icon name="questionCircle" size="md" class="text-primary-600 dark:text-primary-300" />
            <div class="mt-4 text-sm font-semibold">配置遇到问题？</div>
            <p class="mt-2 break-words text-xs leading-5 text-gray-500 dark:text-dark-400">
              {{ contactInfo || '请联系站点管理员' }}
            </p>
          </div>
        </div>
      </aside>

      <main class="min-w-0 px-4 py-8 sm:px-8 lg:px-14 lg:py-12 xl:px-20">
        <section class="border-b border-gray-200 pb-12 dark:border-dark-700">
          <div class="mb-8 flex items-center gap-3 font-mono text-xs font-semibold text-primary-600 dark:text-primary-400">
            <span class="h-px w-8 bg-primary-500"></span>
            BIN API DOCS / 01
          </div>

          <div class="grid items-center gap-10 xl:grid-cols-[minmax(0,1fr)_320px]">
            <div class="min-w-0">
              <h1 class="font-serif text-5xl font-normal tracking-normal text-gray-950 sm:text-6xl dark:text-white">使用教程</h1>
              <p class="mt-6 max-w-2xl text-base leading-8 text-gray-600 sm:text-lg dark:text-dark-300">
                从本地环境到 AI 编程工具，按步骤接入 {{ siteName }} 统一 API。只需一枚密钥，即可开始调用已开放的模型。
              </p>

              <div class="mt-8 max-w-lg overflow-hidden rounded-lg border border-gray-800 bg-gray-950 text-gray-100 shadow-lg">
                <div class="flex items-center justify-between border-b border-gray-800 bg-gray-900 px-4 py-2.5">
                  <span class="font-mono text-[11px] text-primary-300">API 接口地址</span>
                  <button
                    type="button"
                    class="inline-flex items-center gap-1.5 rounded-md bg-gray-800 px-2.5 py-1 text-xs text-gray-200 hover:bg-gray-700"
                    @click="copyApiUrl"
                  >
                    <Icon :name="copiedApiUrl ? 'check' : 'copy'" size="sm" />
                    {{ copiedApiUrl ? '已复制' : '复制' }}
                  </button>
                </div>
                <div class="break-all px-4 py-3 font-mono text-sm">{{ apiBaseUrl }}</div>
              </div>

              <router-link to="/keys" class="btn btn-primary mt-3 px-6 py-3 text-sm shadow-lg shadow-primary-500/20">
                获取 API Key
            ...
            <div class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm dark:border-dark-700 dark:bg-dark-900">
              <div class="flex items-center justify-between border-b border-gray-200 pb-4 text-xs text-gray-400 dark:border-dark-700 dark:text-dark-500">
                <span>开始之前</span>
                <span>约 5 分钟</span>
              </div>
              <ol class="divide-y divide-gray-200 dark:divide-dark-700">
                <li v-for="(item, index) in preparationSteps" :key="item.title" class="flex items-start gap-4 py-4">
                  <span class="flex h-8 w-8 flex-none items-center justify-center rounded-full bg-primary-100 text-xs font-semibold text-primary-700 dark:bg-primary-900/40 dark:text-primary-300">{{ index + 1 }}</span>
                  <div class="min-w-0">
                    <div class="text-sm font-medium">{{ item.title }}</div>
                    <div class="mt-1 text-xs text-gray-400 dark:text-dark-500">{{ item.description }}</div>
                  </div>
                </li>
              </ol>
            </div>
...
let copiedApiUrlTimer: ReturnType<typeof setTimeout> | undefined
...
const preparationSteps = computed(() => [
  { title: `准备 ${siteName.value} 账户`, description: '注册并登录控制台' },
  { title: '创建 API Key', description: '选择分组并妥善保管' },
  { title: '选择接入方式', description: 'Claude Code、Codex 或 API' },
])
...
async function copyApiUrl() {
  ...
  if (copiedApiUrlTimer) clearTimeout(copiedApiUrlTimer)
  copiedApiUrlTimer = setTimeout(() => { copiedApiUrl.value = false }, 1600)
}
